...
- S.E.G.A. Queuing Strategy (Jan 9, 2026): Updated SegaService to use a two-phase execution strategy: Phase 1 queues all structured fields (interleaved), and Phase 2 queues all Lorebooks (interleaved). Lorebook discovery and queueing are explicitly deferred until Phase 1 is complete, satisfying the "Wait to discover" requirement. The display order of Lorebooks is now interleaved to match their execution order.
