...
- **Streaming Optimization (Jan 2026)**: Updated `StoryManager` (`setFieldContent`, `updateDulfsItem`, `saveFieldDraft`) to support a `"none"` save option. This prevents excessive "Auto-saved global story data" logging and disk writes during streaming generation, while keeping the in-memory state updated for the UI. Persistence is now only triggered at the completion of generation.
