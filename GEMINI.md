...
## Historical Memories (Jan 2026)

- **Refactoring**:
    - Extracted `GenX` and `BudgetTimer` to `lib/`.
    - Removed `Review` and `Refine` stages in favor of direct-to-field streaming.
    - Unified `FieldGenerationService` and `ListGenerationService`.
    - Moved DULFS logic to `DulfsService` and Parsing to `ContentParsingService`.
- **UI Improvements**:
    - Fixed Brainstorm UI update delays.
    - Implemented "Streaming Patcher" for Review stage (now deprecated/removed but concept remains).
    - Added "Eye" icon for DULFS Category Summaries.
- **UX Tuning**:
    - Adjusted DULFS prompts to separate List and Content generation instructions.
    - Implemented robust cancellation logic for budget waits.
    - Added Assistant Prefill for DULFS content to enforce format (`Name: ...`).