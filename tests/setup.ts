  ...
  v1: {
    ...
    createRolloverHelper: vi.fn(
      (config: {
        ...
        return {
          add: vi.fn(
            async (item: string | { content: string; tokens?: number }) => {
          ...
          read: vi.fn(() => {
            ...
            let start = 0;
            while (
              total > config.maxTokens + config.rolloverTokens &&
              start < items.length
            ) {
    ...
    tokenizer: {
      encode: vi.fn((text: string) =>
        Promise.resolve(new Array(Math.ceil(text.length / 4))),
      ),
