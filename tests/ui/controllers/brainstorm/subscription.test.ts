...
describe("Brainstorm Subscription Optimization", () => {
  const getMessages = (state: any) => {
    return (state.story.fields[FieldID.Brainstorm]?.data?.messages ||
      []) as BrainstormMessage[];
  };
  ...
  it("should not notify subscriber when only content changes (streaming)", () => {
    ...
    store.dispatch(
      brainstormAppendToMessage({ messageId: "msg-1", content: " World" }),
    );
