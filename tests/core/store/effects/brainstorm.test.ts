...
describe("Brainstorm Effects", () => {
  ...
  it("should handle message editing flow correctly", async () => {
    ...
    const msgB = { id: "msg-b", role: "assistant" as const, content: "Content B" };
    ...
    (api.v1.storyStorage.get as any).mockResolvedValueOnce("Content A Modified");

    store.dispatch(uiBrainstormMessageEditBegin({ id: msgB.id }));
    await wait();

    // Verify A saved
    const updatedA = store.getState().brainstorm.messages.find(m => m.id === msgA.id);
    ...
    (api.v1.storyStorage.get as any).mockResolvedValueOnce("Content B Modified");

    store.dispatch(uiBrainstormMessageEditEnd());
    await wait();

    // Verify B saved
    const updatedB = store.getState().brainstorm.messages.find(m => m.id === msgB.id);
