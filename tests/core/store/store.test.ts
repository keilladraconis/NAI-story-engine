...
describe("Redux Store", () => {
  ...
  it("should update field content", () => {
    const store = createStore(rootReducer, initialRootState);
    store.dispatch(
      fieldUpdated({ fieldId: FieldID.StoryPrompt, content: "New content" }),
    );
  ...
  it("should notify subscribers", () => {
    ...
    store.dispatch(
      fieldUpdated({ fieldId: FieldID.StoryPrompt, content: "Change" }),
    );
  ...
  it("should reset entire state on STORY_CLEARED", () => {
    ...
    // Dirty the state
    store.dispatch(
      fieldUpdated({ fieldId: FieldID.StoryPrompt, content: "Dirty Content" }),
    );
    store.dispatch(uiSectionToggled({ id: FieldID.StoryPrompt }));
    store.dispatch(
      generationRequested({ id: "1", type: "field", targetId: "test" }),
    );

    // Verify dirty
    let state = store.getState();
    expect(state.story.fields[FieldID.StoryPrompt].content).toBe(
      "Dirty Content",
    );
