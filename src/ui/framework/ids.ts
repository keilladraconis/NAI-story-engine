export const IDS = {
  CRUCIBLE: {
    WINDOW_ROOT: "cr-root",
    STRATEGY_ROW: "cr-strategy-row",
    NODES_COL: "cr-nodes-col",
    PHASE_TEXT: "cr-phase",
    DEEPEN_BTN: "cr-deepen-btn",
    COMMIT_BTN: "cr-commit-btn",
    SEED_BTN: "cr-seed-btn",
    ...
    node: (id: string) => ({
      ...
      SERVES: `cr-node-${id}-serves`,
      NAME: `cr-node-${id}-name`,
    }),
    strategy: (s: string) => `cr-strat-${s}`,
    kindGroup: (kind: string) => `cr-kind-${kind}`,
  },
  BRAINSTORM: {
    ROOT: "se-bs-root",
    LIST: "se-bs-list",
