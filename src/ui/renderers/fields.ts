...
import { FieldConfig, FieldID, DulfsFieldID } from "../../config/field-definitions";
...
import { createHeaderWithToggle, createToggleableContent, createResponsiveGenerateButton } from "../ui-components";

const { row, column, text, button, checkboxInput, textInput, collapsibleSection } = api.v1.ui.part;
...
const renderTextField = (config: FieldConfig, state: RootState): UIPart => {
    ...
    const request = state.runtime.queue.find(r => r.id === requestId) || 
                    (state.runtime.activeRequest?.id === requestId ? state.runtime.activeRequest : null);
    ...
    const onToggle = () => {
        if (isEditing) {
            // Save on exit
            dispatch(fieldUpdated(config.id, draft));
        } else {
            // Init draft on enter
            dispatch(uiInputChanged(draftKey, content));
        }
        dispatch(uiEditModeToggled(config.id));
    };
    ...
    return collapsibleSection({
        ...
        content: [
            createHeaderWithToggle(config.description, isEditing, onToggle, genButton),
...
const renderListField = (config: FieldConfig, state: RootState): UIPart => {
    ...
    const summaryDraft = state.ui.inputs[summaryDraftKey] !== undefined ? state.ui.inputs[summaryDraftKey] : summary;

    // List Generation
    const listGenId = `gen-list-${config.id}`;
    const isListGenRunning = state.runtime.activeRequest?.id === listGenId || state.runtime.queue.some(r => r.id === listGenId);
    ...
    const summaryUI = column({
        ...
        content: [
            row({
                ...
                content: [
                    text({ text: "Category Summary", style: { "font-weight": "bold", opacity: "0.7" } }),
                    button({
                         iconId: isSummaryEditing ? "save" : "edit-3",
                         style: { width: "24px", padding: "4px" },
                         callback: () => {
                             if (isSummaryEditing) {
                                 dispatch({ type: 'story/dulfsSummaryUpdated', payload: { fieldId: config.id, summary: summaryDraft } });
                             } else {
                                 dispatch(uiInputChanged(summaryDraftKey, summary));
                             }
                             dispatch(uiEditModeToggled(summaryEditModeKey));
                         }
                    })
                ]
            }),
    ...
    const itemsUI = list.map(item => {
        ...
        const draftName = state.ui.inputs[itemDraftKey] !== undefined ? state.ui.inputs[itemDraftKey] : item.name;
        
        const itemGenId = `gen-item-${item.id}`;
        const isItemGenRunning = state.runtime.activeRequest?.id === itemGenId || state.runtime.queue.some(r => r.id === itemGenId);
        ...
        return row({
            ...
            content: [
                isEditing ? textInput({
                    ...
                    onChange: (val) => dispatch(uiInputChanged(itemDraftKey, val)),
                    style: { flex: 1 }
                }) : text({
                    text: item.name,
                ...
                createResponsiveGenerateButton(
                    ...
                    {
                        onStart: () => dispatch(generationRequested({
                            id: itemGenId,
                            type: 'field',
                            targetId: `${config.id}:${item.id}`
                        })),
                        onCancel: () => dispatch(generationCancelled(itemGenId))
                    },
                    ""
                ),
                ...
                button({
                    ...
                    callback: () => {
                        if (isEditing) {
                            dispatch(dulfsItemUpdated(config.id as DulfsFieldID, item.id, { name: draftName }));
                        } else {
                            dispatch(uiInputChanged(itemDraftKey, item.name));
                        }
                        dispatch(uiEditModeToggled(itemEditKey));
                    }
                }),
                button({
                    iconId: "trash",
                    style: { width: "24px", padding: "4px" },
                    callback: () => dispatch(dulfsItemRemoved(config.id as DulfsFieldID, item.id))
                })
            ]
        });
    });
    ...
    return collapsibleSection({
        ...
        storageKey: `story:kse-section-${config.id}`,
        content: [
            text({ text: config.description, style: { "font-style": "italic", opacity: 0.8 } }),
            summaryUI,
                ...
                content: [
                    createResponsiveGenerateButton(
                        ...
                        { isRunning: isListGenRunning },
                        {
                            ...
                            onCancel: () => dispatch(generationCancelled(listGenId))
                        },
                        "Generate Items"
                    ),
                    button({
                        ...
                        iconId: "plus",
                        callback: () => {
                            dispatch(dulfsItemAdded(config.id as DulfsFieldID, {
                                id: api.v1.uuid(),
