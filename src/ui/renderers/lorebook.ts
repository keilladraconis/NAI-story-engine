...
export const renderLorebookPanel = (state: RootState): UIExtensionLorebookPanel => {
    ...
    if (entryId) {
        ...
        if (foundItem && foundFieldId) {
            ...
            const isGenerating = state.runtime.activeRequest?.id === genId || state.runtime.queue.some(r => r.id === genId);
            ...
            content = [
                column({
                    ...
                    content: [
                        column({
                            content: [
                                text({ text: `Source: ${foundItem.name}`, style: { "font-weight": "bold", opacity: 0.8 } }),
                                text({ text: foundItem.content, style: { "font-style": "italic", "font-size": "0.9em" } })
                            ],
                        ...
                        createHeaderWithToggle(
                            ...
                            () => {
                                if (isEditing) {
                                    ...
                                    dispatch({ 
                                        type: 'story/dulfsItemUpdated', 
                                        payload: { fieldId: foundFieldId, itemId: foundItem!.id, updates: { content: draft } } 
                                    });
                                } else {
                                    dispatch(uiInputChanged(draftKey, itemContent));
                                }
                                dispatch(uiEditModeToggled(`lorebook:${entryId}`));
                            },
                            createResponsiveGenerateButton(
                                ...
                                { isRunning: isGenerating },
                                {
                                    ...
                                    onCancel: () => dispatch(generationCancelled(genId))
                                },
                                "Generate"
                            )
                        ),
        ...
        content = [
            column({
                style: { padding: "16px", "text-align": "center", opacity: 0.6 },
                content: [
                    text({ text: "Select an entry in the Lorebook to begin." })
                ]
            })
        ];
    }
