...
export type SeChatHeaderOptions = SuiComponentOptions<
  SeChatHeaderTheme,
  SeChatHeaderState
>;
...
> {
  ...
  constructor(options: SeChatHeaderOptions) {
    ...
    this._summarizeBtn = new SeConfirmButton({
      ...
      onConfirm: async () => {
        store.dispatch(uiBrainstormSummarize());
      },
    });
  }
  ...
  async compose(): Promise<UIPartRow> {
    ...
    this._watcher.watch(
      (s) => {
        ...
        return {
          title: c.title,
          mode: (c.mode || "cowriter") as BrainstormMode,
        };
      },
      ({ title, mode }) => {
    ...
    return row({
      ...
      content: [
        ...
        button({
          ...
          callback: () => {
            store.dispatch(modeChanged("cowriter"));
          },
        }),
        button({
          id: "se-bs-mode-critic",
          ...
          callback: () => {
            store.dispatch(modeChanged("critic"));
          },
        }),
        summarizePart,
        button({
