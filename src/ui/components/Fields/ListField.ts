...
const events = createEvents({
    toggleSummaryEdit: (props: ListFieldProps, summary: string, draft: string, isEditing: boolean) => {
        if (isEditing) {
            dulfsSummaryUpdated({ fieldId: props.config.id, summary: draft });
        } else {
            uiInputChanged({ id: `summary-draft-${props.config.id}`, value: summary });
        }
...
export const ListField: Component<ListFieldProps, RootState> = {
    ...
    describe(props, state) {
              ...
              row({
                ...
                content: [
                  ...
                  button({
                    ...
                    callback: () => events.toggleSummaryEdit(props, summary, summaryDraft, isSummaryEditing)
                  }),
                ],
              }),
        ...
        const itemsUI = list.map(item => ListItem.describe({ config, item }, state) as UIPart);
    ...
    bind(ctx, props) {
        ...
        useSelector(
            ...
            (slice) => {
                 ...
                 const itemsUI = list.map(item => 
                     ListItem.describe({ config, item, dispatch }, slice.state) as UIPart
                 );
        ...
        useSelector(
            ...
            (slice) => {
                ...
                const draft = slice.summaryDraft !== undefined ? slice.summaryDraft : summary;
                
                updateParts([
                    {
                        id: `summary-toggle-${config.id}`,
                        iconId: isEditing ? "save" : "edit-3",
                        callback: () => events.toggleSummaryEdit(props, summary, draft, isEditing)
                    },
                    {
