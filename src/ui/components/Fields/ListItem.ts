...
const events = createEvents({
    toggleEdit: (props: ListItemProps, draftName: string, isEditing: boolean) => {
        if (isEditing) {
            ...
            uiInputChanged({ id: `item-name-${props.item.id}`, value: props.item.name });
        }
...
export const ListItem: Component<ListItemProps, RootState> = {
    ...
    describe(props, state) {
        ...
        if (!state) return row({ id: `item-row-${item.id}`, content: [text({ text: item.name })] });
    ...
    bind(ctx, props) {
        ...
        GenerationButton.bind(ctx, {
            id: `btn-${requestId}`,
            requestId,
            request: { id: requestId, type: "field", targetId: `${props.config.id}:${props.item.id}` },
            label: "",
            style: { padding: "4px" },
            dispatch
        });
        ...
        useSelector(
            state => ({
                ...
                itemName: (state.story.dulfs[props.config.id as DulfsFieldID] || []).find(i => i.id === item.id)?.name,
            }),
            (slice) => {
                ...
                updateParts([
                    { id: `item-input-${item.id}`, style: { display: isEditing ? "block" : "none" } },
                    { id: `item-text-${item.id}`, style: { display: isEditing ? "none" : "block" }, text: slice.itemName }
                ]);
            }
        );
    }
