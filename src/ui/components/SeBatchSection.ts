...
> {
  ...
  private async _rebuildEntityList(): Promise<void> {
    ...
    api.v1.ui.updateParts([
      { id: B.ENTITY_LIST, content: parts } as unknown as Partial<UIPart> & {
        id: string;
      },
    ]);
  }
  ...
  async compose(): Promise<UIPartCollapsibleSection> {
    ...
    this._watcher.watch(
      ...
      (title) => {
        api.v1.ui.updateParts([
          { id: B.SECTION, title } as unknown as Partial<UIPart> & {
            id: string;
          },
        ]);
      },
    ...
    const reforgeBtn = button({
      ...
      callback: () => {
        store.dispatch(batchReforgeRequested({ batchId }));
      },
    });
