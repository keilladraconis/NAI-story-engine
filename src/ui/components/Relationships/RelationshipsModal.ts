...
export async function openRelationshipsModal(
  ctx: BindContext<RootState> | RelModalCtx,
): Promise<void> {
  ...
  async function rebuild(): Promise<void> {
    ...
    if (allRels.length === 0 && !addFormVisible) {
      ...
      // Collect entities that appear as "from" (outgoing relationships)
      ...
      // Also show relationships where entity only appears as "to" (no outgoing rels)
