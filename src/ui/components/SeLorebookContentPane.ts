...
> {
  ...
  private _buildRelRow(rel: Relationship): UIPart {
    ...
    const fromName =
      entities.find((e) => e.id === rel.fromEntityId)?.name ?? "?";
    ...
    return row({
      ...
      content: [
        text({
          text: `${fromName} → ${toName}: ${rel.description}`,
          style: S.relText,
        }),
        button({
          id: R.DELETE_BTN,
          ...
          callback: () => {
            store.dispatch(relationshipRemoved({ relationshipId: rel.id }));
          },
        }),
      ],
    });
            ...
            button({
              ...
              callback: () => {
                if (this._relFormVisible) {
                  this._relFormVisible = false;
                  api.v1.ui.updateParts([
                    { id: L.REL_FORM, style: S.relFormHidden },
                  ]);
                  ...
                  api.v1.ui.updateParts([
                    { id: L.REL_FORM, style: S.relFormVisible },
                    {
                      ...
                      text: this._availableEntities[0]?.name ?? "(no entities)",
                    },
                  ]);
                }
              },
          ...
          content: [
            button({
              ...
              callback: () => {
                ...
                this._targetIdx =
                  (this._targetIdx + 1) % this._availableEntities.length;
                api.v1.ui.updateParts([
                  {
                    id: L.REL_FORM_TARGET_BTN,
