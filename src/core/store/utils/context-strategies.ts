import { StoryState, DulfsItem, GenerationRequest } from "../types";
import {
  FieldID,
  FIELD_CONFIGS,
  LIST_FIELD_IDS,
...
Keep the description concise and focused on narrative potential.`,
    ...
    return {
      ...
      filters: config?.filters
        ? config.filters.map((f) => Filters[f]).filter((f) => !!f)
        : [],
    };
  }
