...
export const lorebookSyncSubscriber = (store: Store<RootState>) => {
    ...
    async function syncItem(state: RootState, fieldId: string, itemId: string) {
        ...
        let catId = (await api.v1.lorebook.categories()).find(c => c.name === catName)?.id;
        if (!catId) {
             catId = api.v1.uuid();
             await api.v1.lorebook.createCategory({ id: catId, name: catName, enabled: true });
        }
        ...
        if (item.lorebookEntryId) {
            ...
            if ((item.content && item.content.length > 0) || (item.name && item.name.length > 0)) {
                ...
                try {
                    ...
                    store.dispatch(dulfsItemUpdated(fieldId as DulfsFieldID, itemId, { lorebookEntryId: newId }));
                } catch (e) {
