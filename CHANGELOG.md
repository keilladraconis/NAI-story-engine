...
- **Refine no longer produces story prose under Xialong mode.** When Xialong was active, refining a Foundation field (Style, Intent, ATTG, Contract) or lorebook entry would write prose continuation instead of the rewritten field. The `[USER] [ Style: ... ]` anchor that field factories append via `appendXialongStyleMessage` was sitting immediately above the `----` divider in the final message array, acting as a prose generation directive that overrode the rewrite instructions. `buildRefineTail` now owns the full message assembly — it strips any trailing `[ Style: ... ]` user message from the base messages before inserting `----`, ensuring the divider cleanly separates field context from the refine prompt. The generation placeholder is also filtered from refine history to prevent a spurious empty assistant message being sent to the API.
