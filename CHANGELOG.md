...
- **A Thread limit, so unfinished business cannot pile up unchecked.** **Setup → Engine → Thread limit** caps how many Threads a story carries at once, default **8**. At the limit, creating another displaces the weakest one rather than being refused: a satisfied Thread goes first, then the shortest horizon, then the oldest — and never the one you just made. Lowering the limit deletes nothing by itself; it takes effect the next time you add a Thread. **The displaced Thread's lorebook entry is left behind, and left switched on.** Story Engine will not delete from your lorebook, and the part of the Engine that reconciles entries it no longer manages is not built yet — so if you work at the limit for a while, expect entries in your lorebook that no Thread in the World points at any more. They are yours to disable or delete; nothing here will touch them.
