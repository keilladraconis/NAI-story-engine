...
- **The Engine watches what you write, and acts on it.** Switched on, it wakes a few seconds after each generation, reads the prose that has appeared since it last looked, and asks a small model what your story has made wrong, left hanging or settled. Then it acts: **rewriting the lorebook entry** of a character or place the prose has changed, **opening a Thread** for a commitment it has raised, **switching off** one it has settled, and **condensing** an entry that has sprawled. It is **off by default** — switch it on at **Engine → On**, along with **Delay (seconds)** before it looks (8), **Minimum new paragraphs** worth a pass (1), the **Thread limit** (8), and **Condense entries over (paragraphs)** (5). **Every setting belongs to the story you set it in**, so turning the Engine on for one story does not turn it on for the next.
- **The Engine never asks you to wait for it.** A pass costs one small generation plus at most one entry rewrite, from the same output budget your own writing draws on. It checks what is left before every action and defers what it cannot afford to the next pass — a deferred rewrite is still made against the prose that called for it. If the budget is short, or you are already generating, it steps aside silently and tries again later rather than queueing. Writing by hand wakes nothing, so the HUD's **⚡** is how you run a pass over prose you typed yourself.
- **You pick the model your creative work runs on.** **Engine → Creative model** chooses between **GLM 4.6**, which every subscription can reach and follows instructions closely, and **Xialong v1**, a creative-writing fine-tune that needs an Opus subscription. It governs everything you read — prose, chat, the Forge, lorebook entry text, the Engine's own rewrites — and, like the rest of the Engine's settings, belongs to the story you set it in. Activation keys and the short internal summaries behind each entity and Thread are extraction work and always run on GLM whichever you pick. This replaces the **Xialong Mode** switch in NovelAI's script settings, which could only ever be read by Story Engine and never written from it; new stories start on GLM, so set this once per story if you have Opus.
- **It edits your lorebook, and keeps no copy of what it replaced.** When the Engine rewrites an entry, the wording you had is gone — nothing stores it, and undo will not bring it back. **If the exact text of an entry matters to you, export your lorebook before switching the Engine on.** What bounds this is how little it reaches for: it only rewrites entries it manages, so an entry no Story Engine entity or Thread is bound to is never touched. It never deletes anything and never changes an entry's category. The strongest thing it does to an entry it no longer wants firing is switch it off — one click to put back.
- **Story Engine's records move forward only.** Undo changes your prose; it does not change what Story Engine has written down. A character, Thread or field created during a scene you have since backed out of is still in the World, and **deleting it is the only thing that takes it out**. The Engine's own work moves forward the same way: an entry it rewrote stays rewritten, a Thread it opened stays open. Two continuations from the same point share one set of notes.
- **Condensing is the one action that can lose information.** A condense rewrites a long entry shorter, and a dropped detail is one nothing downstream will notice is missing. Three checks stand behind it: a result that is not actually shorter is thrown away, one under a third of the entry's length is thrown away as a summary rather than a compaction, and one that ran out of room mid-rewrite is thrown away rather than trimmed. A rejected condense leaves the entry exactly as it stood, and that entry is not offered again until it has grown by another paragraph. Length alone triggers it, and you own the number: **Engine → Condense entries over (paragraphs)**, default 5, range 2–10.
- **A one-line Engine HUD you cannot lose.** A second panel showing one fixed line, meant to be read as a shape: an icon for what the Engine is doing (off, watching, reading, working, waiting on budget, or stuck), then three counts — paragraphs not yet read, Threads still **open**, and rewrites made to your lorebook since you opened the story — then four bars of remaining output budget, and a **⚡** to run a pass on the spot. Every slot carries a tooltip saying what its number counts. The ⚡ does nothing while the Engine is off, and pressing it mid-pass is ignored rather than starting a second one. The panel can be minimised but never closed, so the Engine can never be quietly running behind a surface you have shut.
- **Threads have a scale and a lifecycle.** A Thread is the story's unfinished business — a promise made, a letter unopened, an argument left hanging. Each now carries a **horizon** for how much of the story it spans, set from three buttons in its edit pane: **Point** for a detail the next few paragraphs should pick up, **Plot** for a subplot that can survive a scene about something else, and **Arc** for the story's spine. Each also has a **status** — open, satisfied, or abandoned — shown on every row in the World list as a hollow circle, a check or a dash. Satisfied Threads stay where you left them rather than being filed away. You set both by hand and the Engine sets them too; the one thing it never does is rescale a Thread, so every Thread it opens is a **Plot**.
- **A Thread now has a lorebook entry that stays quiet while you are still writing about it.** Filed under **SE: Threads**, holding a sentence or two about what is outstanding. It is not keyed on words — a condition decides when it speaks. While your prose keeps naming the Thread's cast or subject, it costs you nothing; once they have gone missing for a while (a couple of paragraphs at Point, about a scene at Plot, about a chapter at Arc) the reminder starts arriving. A new Thread gets one quiet window first, so its first verdict is about prose written after the commitment existed. **Every named Thread gets an entry** — from the Forge as it is created, from the World's **+** once you give it a title and Save. An untitled Thread gets nothing, so abandoning the pane leaves no stray entry behind.
- **Threads now retire themselves, including ones the story walked away from.** When your prose settles a commitment, the Engine marks that Thread satisfied and switches its entry off — the reminder stops, rather than the model being told a plot is over. A Thread nothing has mentioned for a long time ages out the same way: ten of its own forgetting windows (roughly 25 paragraphs at Point, 100 at Plot, 300 at Arc). Any pass that finds it in your prose pushes that clock back, so a Thread you are circling never ages; a Thread you made by hand starts with no clock and gets one when your prose first mentions it. **A Thread that ages out is marked _abandoned_, not satisfied**, with its own glyph — a check against a commitment you never resolved would be telling you something untrue about your own story. Abandoned behaves as satisfied does: the entry goes quiet, the slot frees, and reopening it is one press. **Deleting a Thread switches its entry off too**, leaving it in your lorebook one switch away from coming back.
- **A Thread limit, so unfinished business cannot pile up unchecked.** **Engine → Thread limit** caps how many Threads a story carries, default **8**. The World's add-Thread button carries the count — `3/8` — and **at the limit it will not create**: it dims, and its tooltip says to satisfy and delete a Thread or raise the limit. A Thread arriving from the Forge or the Engine does displace, because that create came with a decision attached: the weakest goes — a satisfied one first, then the shortest horizon, then the oldest — and never the one just made. Lowering the limit deletes nothing; it applies to the next Thread added. A displaced Thread's title and notes leave the World, but **its entry is switched off as it goes**, and the entry itself stays in your lorebook untouched.

### Changed

- **Story Engine's sidebar is now three tabs: Setup, Engine, and Chat.** Setup is where a story begins — the narrative Foundation (Shape, Intent, Story Contract, ATTG, Style and Intensity) with the **opening scene** control and the **Import Wizard**. The header keeps the generation status widget and the S.E.G.A. progress line. The Engine tab holds the Forge, the Engine's own controls, and the World with its Threads — the Engine's own On/Off switch and settings sit between them, collapsed under an **Engine** header. Opening a story with nothing written lands you on Setup; opening one in progress lands you on Engine.
- **The Setup tab leads you through building a story rather than showing everything at once.** Import Wizard, then Intensity, then a prompt to talk the story through, then the Foundation, then the opening scene, then the Engine's settings. The Foundation sits in a collapsible section that starts closed and opens once a brainstorm has something in it, then stays however you leave it. The opening scene is a card that appears once the Foundation has content and the story is still blank.
- **"Talk it through" opens a fresh brainstorm, and appears when you need it.** It sits under Intensity while there is nothing to work from and steps aside once there is — a Foundation field filled in, or a brainstorm you have talked in. Until you have picked an Intensity it waits and points back at the picker, since the brainstorm writes in the register you set.
- **Keys and summaries are generated by the model that is good at them.** Activation keys and the short internal summaries behind each entity and Thread are extraction work, so they always go to the instruction-following model, even with Xialong Mode on — which means fewer key lists arriving with commentary attached. Everything you read is still written by the prose model: the opening scene, lorebook entry text, chat, the Forge, and the Foundation fields.
- **Story Engine Debug now does one job.** The list each pass produces goes to the script log when **Story Engine Debug** is on in NovelAI's script settings; with it off, the HUD is the whole surface. The per-action store log has moved to its own setting, **Log every store action**, off by default and only worth touching if you are debugging Story Engine's state.

### Fixed

- **Threads are their own list, and the finished ones fold away.** A Thread used to wrap its cast — the entities it names were rendered inside it and hidden from the World above, so a thread was the only way to reach them. The World now lists every entity once, and Threads sit in their own section beneath it as one line each. **Satisfied and abandoned Threads collapse behind a "Retired (n)" row**, open by one click, so a story that has settled twenty commitments does not bury the three it still owes. A Thread's cast has not gone anywhere — it is what the Thread's lorebook entry watches your prose for — it is edited in the Thread's own pane rather than being a filing system for the World. The greyed-out lorebook toggle on each Thread row is gone; it was a placeholder for always-on Thread entries, which the forgetting detector replaced.
- **"Talk it through" opens the empty brainstorm you already have.** Story Engine starts every story with an empty **Brainstorm 1** selected, and the button made a second chat anyway — so a first click landed you in **Brainstorm 2** with an untouched **Brainstorm 1** sitting beside it in Sessions. It now reuses the selected chat when nothing has been said in it, and still starts a fresh one when there is a conversation to keep.
...
- **Continue Scene is gone.** Continuing a scene is what the editor is for. Writing the opening from your Foundation is unchanged; once there is prose on the page, Story Engine leaves the writing to you.
- **Story Engine's saved state does not carry over from 0.14.x.** Story Engine is alpha and there is no migration, so on the first open after upgrading the World, the Foundation and your saved brainstorms are empty — **including every Thread you have created**. Your story text, lorebook, Memory and Author's Note are untouched; what resets is Story Engine's own record of which entries it manages, and the Import Wizard picks that back up. The **Xialong Mode** script setting is gone — the creative model is chosen per story in Setup now.
