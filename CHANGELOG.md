...
- **The ATTG and Author's Note sync switches respond to touch.** On phones and tablets these toggles were very hard to turn on or off: one tap could deliver two clicks, flipping the switch to its new setting and straight back, so it looked stuck and the Memory / Author's Note write ran twice. Foundation's sync toggles and ⚡, and thread membership toggles, now ignore a repeat click arriving inside the same tap.
