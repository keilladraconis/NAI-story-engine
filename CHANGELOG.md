...
- **Opening Scene asks how the story should open.** The header button no longer fires the moment you click it — it opens a box asking _"Open the story with:"_. Name a technique (_"in media res"_), a beat (_"the protagonist's first trauma"_), or a literal moment, and the opening is written from inside it, outranking the engine's default choice of where to start; leave it blank for exactly the opening the button wrote before. Your direction is kept with the story, so undoing the opening and clicking again brings it back for another pass.
- **The Import wizard imports the Story Contract.** The Contract was the one Foundation field import silently skipped, leaving it to fill in by hand. **Import All** now writes it too — after Shape and Intent, so it's drafted from what the same import just produced — and there's a **Contract** button in the Story row beside them.
- **The entity edit pane's Content ⚡ is adaptive.** Like the Foundation fields, it generates lorebook content on an empty entry and opens a refine chat once content exists.

### Changed

- **Story Engine's interface is rebuilt on NovelAI's JSX/Preact UI API.** The whole sidebar panel — Foundation, Chat, World and Threads, Forge, the Opening/Continue Scene header, and the Import wizard — now renders reactively from the shared store and takes its colors from the active theme. The homebrew SUI framework and its vendored library are gone, and Story Engine is a single sidebar panel again (no parallel "(JSX)" tab). The panel updates faster and more reliably, a whole class of dropped-callback bugs went out with the old render path, and the bundle is 30% smaller. Every workflow behaves as before — this is a foundations change.
- **One place to drive generation.** The header is now always visible — on the Chat tab, in an entity's edit pane, and over the Import wizard — showing remaining output budget when idle, Cancel while generating, Continue when a generation is waiting on you, and a live countdown while it waits on budget, with SEGA progress on its own line beneath. Every other generate button simply dims while its own work is in flight, so the chat composer no longer carries a second, half-working copy of these controls. Cancelling from the header stops everything in flight at once.
- **Deleting or retrying a chat message takes two taps.** Both destroy work with no undo — Delete removes the turn, Retry throws away every turn after the one you retried — so each arms on the first tap (an amber warning icon) and only acts on a second within four seconds. Delete also moved out to the far right edge of the bubble, clear of Edit and Retry.

### Fixed

- **Generated and refined text no longer stops mid-sentence.** A generation that ran out of room was saved exactly where the cap fell — a lorebook entry breaking off mid-word, a refined field committing the fragment it had reached. Both now continue automatically whenever the model was cut off rather than finished, carrying on to a natural end or a generous ceiling; an entry that fits in one pass still costs a single generation. Refining an entry also gets a budget sized to what it's rewriting (it previously had less than half the room the entry was written with, so a full rewrite could never fit) and no longer treats a closing bracket as the end, so prose containing a bracketed aside survives intact.
- **A tap does one thing on phones and tablets.** A single touch could arrive as two clicks, and nearly every control was exposed: Send cleared the composer without posting the message, two-step confirms armed and fired in one go, sync switches and expanders applied a change and instantly undid it, and anything that creates something made two — a stray `(unnamed)` entity, an empty thread, a duplicate lorebook entry, a second forge or summary session, a whole import batch run twice against the same output budget. Because lists are patched positionally, a doubled tap on a chat bubble could also delete the neighbouring message or retry the wrong turn. Every control now ignores a repeat click inside the same tap, and the work underneath is guarded independently, so nothing can queue the same generation twice however it was asked for: a lorebook entry can only belong to one entity, a Foundation field refuses a second generation while one is queued, the entity ⚡ skips pieces already in flight (including ones S.E.G.A. is working on), Forge refuses to open a second session while one is starting, and asking for a summary already being written brings you back to it.
- **The Forge no longer litters your lorebook with duplicate categories.** Committing a forge session casts every draft at once, and each one created its own `SE: Characters` (or `SE: Locations`, …) instead of sharing one — a single commit could leave six identically-named categories behind with the entities scattered across them. Casting is now a true find-or-create, and Story Engine tidies up on load, folding duplicate `SE:` categories an earlier build left behind into one and moving their entries across. Your own categories are never merged, even if two share a name.
- **Generation can be un-stalled again.** NovelAI's script harness pauses token replenishment when it can't see recent user activity, and clicks inside a script's own panel didn't count — so the output budget drained and never refilled, and the Continue button was decorative. Working in the panel now keeps the budget topping up, and one press on the header's status widget resumes a stalled generation.
- **An unsent chat message is no longer lost.** Switching to the Story Engine tab tore down the chat panel and took the composer's contents with it. Unsent text is now kept per chat and saved with the story, so it survives tab switches, session switches, and reloads — and each session keeps its own, so switching chats no longer shows you the previous one's half-written message.
- **The Story Contract generates a contract instead of more story.** Asking for a Story Contract returned a passage of prose rather than the Required / Prohibited / Emphasis lines: the request was assembled entirely from background context with no actual question at the end of it, so the model continued the story text sitting in that context. It's now asked outright and starts its reply from the `REQUIRED:` label, so it can only answer in the contract's three-line form.
- **The chat's Wait button counts down again.** When a generation stalls on the output-token budget the Send button becomes ⚠️ Continue; clicking it left a frozen "⏳ Wait" with no sense of how long remained. It now ticks down each second, matching the header's status readout.
