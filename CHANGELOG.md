...
- **Forge is now a chat.** Clicking the Forge button opens a typed chat session in the Chat tab — phase indicator (Sketch → Expand → Weave), inline draft cards beneath each assistant turn, and Cast All / Discard All header controls. The previous one-shot loop is gone; you can talk to the forge mid-pass, send empty messages to advance phases, and revisit the same session by clicking Forge again.
- **Inline entity cards in the chat body.** Drafts created or revised by a forge turn render as cards beneath the message that affected them, with their own Cast / Discard / Edit buttons. Each chat type spec can opt in by implementing `inlineEntityIdsFor`.
...
- **The forge chat is now agentic.** Typing in the input _discusses_ with the forge — it answers conversationally and only emits actions for what you explicitly ask (e.g. "add a dockmaster named Halloran"). Running the semi-automatic expansion pass moved to a **Forge Ahead** button in the header; an empty send no longer forges. A **🧹 Scrub** header indicator appears when discards have queued a reference-cleanup and runs it on demand without a full pass. The send button is now "Send".
- **Forge turns render as interleaved prose and action chips.** Each action the forge takes — create, revise, delete, rename, thread, and its running critique — shows as a compact chip inline, in the order the forge performed it, so its narration stays attached to the action it precedes (the Claude-Code-style turn). The raw `[CREATE …]` command syntax is no longer shown; the draft card still appears below the message as the expandable detail for creates.
...
- **Cast All / Discard All end the forge session.** They are the explicit close: the chat (and its tombstones / pending scrub) is dropped, so the next Forge starts fresh. While a session is open, the Forge button resumes it and feeds your guidance box in as a chat message instead of silently dropping it. Cast All also drops you on the Story Engine tab with the World section expanded, so the freshly-cast entities are right in front of you.
...
- **The chat input adapts to the active chat type.** A forge session shows the placeholder "Influence the next forging step", a "Continue Forging" send button, and no Clear button; brainstorm/summary keep the generic input. Driven by new `inputPlaceholder` / `sendLabel` / `showClearButton` fields on `ChatTypeSpec`.
...
- **One entity card across the forge chat and the World.** Draft cards in the forge chat now use the same card as live World entities — a header row with the category icon, a click-to-edit name, and lifecycle-appropriate actions (draft: Cast + Discard; live: Generate) — replacing the old flat Edit/Cast/Discard text buttons. Drafts keep a tinted left border and a chat indent; `SeInlineEntityCard` is removed. Discarding a manual "+ Add Entity" draft (which has no forge session) now simply deletes it.
