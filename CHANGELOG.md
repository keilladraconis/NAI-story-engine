...
- **An unsent chat message survives a trip to the Story Engine tab.** Switching tabs tore down the chat panel and took the composer's contents with it, so a long setup message written but not yet sent was simply gone on the way back. Unsent text is now held per chat outside the panel and restored when you return — each session keeps its own, so switching chats no longer shows you the previous one's half-written message. (Held for the life of the script, so it survives tab and session switches but not a page reload.)
