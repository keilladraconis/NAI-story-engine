...
- **Story Engine's sidebar is now three tabs: Setup, Engine, and Chat.** Setup is where a story begins — the narrative Foundation (Shape, Intent, Story Contract, ATTG, Style and Intensity) with the **opening scene** control and the **Import Wizard**. The header keeps the generation status widget and the S.E.G.A. progress line. The Engine tab holds the Forge, the Engine's own controls, and the World with its Threads — the On/Off switch sits under the Forge where you can always see it, with the settings collapsed beneath it and the World below. Opening a story with nothing written lands you on Setup; opening one in progress lands you on Engine.
