...
- **Refining no longer stops mid-sentence.** A refine ran as one capped generation, so a rewrite that needed more room than the cap allowed was simply cut off wherever it had reached — and the truncated text is what Commit wrote back. It hit lorebook entries hardest, since an entry is written with more than twice the room a refine was given, so a full rewrite could never fit. Refine now continues automatically whenever the model was cut off by the cap rather than finishing, carrying on until the rewrite reaches a natural end or a generous ceiling. Lorebook rewrites also get a budget sized to the entries they rewrite, and no longer treat a closing bracket as the end of the entry — so prose containing a bracketed aside survives intact.
