...
- **The Engine watches what you write and records what needs attention — and does nothing else yet.** Switched on, it wakes a few seconds after each generation, reads whatever prose has appeared since it last looked, and asks a small model one question: what in the recorded World has your story now made wrong, left hanging, or settled? The answer goes to the script log as a short list — revise this character, open this unresolved commitment, retire this thread — and that is where it stops. **Nothing is written, created, or retired.** Your lorebook, your entities and your Threads are untouched; this release ships the watching half deliberately, so you can read what the Engine would do for a few sessions before it is allowed to do any of it. It is **off by default** — turn on **Engine** (`engine_enabled`) in Story Engine's script settings, where you can also change how long after a generation it looks (8 seconds) and how many new paragraphs are worth a pass (1). A pass costs one small generation out of the same output budget as everything else, and skips itself when there is nothing new to read.
- **A one-line Engine HUD you cannot lose.** A second panel, **Engine HUD**, shows a single fixed line meant to be read as a shape rather than as words: an icon for what the Engine is doing (switched off, watching, reading, working, waiting on output budget, or stuck), how many paragraphs it has not read yet (`12¶`), how many Threads the story has (`⚑5`), how many entities it has revised on this branch (`∆0` — always zero while it only watches), four bars of remaining output budget, and a **⚡** that runs a pass on the spot. The ⚡ is what you need when you are typing by hand, since nothing else wakes the Engine — writing yourself produces no generation. It does nothing while the Engine is off, and pressing it during a pass is ignored rather than starting a second one. The panel can be minimised but never closed, which is the point: the Engine can never be quietly running behind a surface you have shut.
