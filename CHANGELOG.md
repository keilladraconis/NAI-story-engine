...
- **The Engine's account of itself is behind Story Engine Debug.** The list each pass produces goes to the script log, and only when **Story Engine Debug** is switched on in NovelAI's script settings — with it off the Engine says nothing at all and the HUD is the whole surface, which is what makes the HUD worth watching. Turn it on when you want the detail: one line per pass and one per intent, naming what the Engine would have done.
