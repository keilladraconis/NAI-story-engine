...
- **Refine no longer produces story prose under Xialong mode.** When Xialong was active, refining a Foundation field (Style, Intent, ATTG, Contract) or lorebook entry would write prose continuation instead of the rewritten field. The root cause was twofold: (1) the field factory's generation directive ("Generate a style guideline…") remained above `----` in the context, acting as the primary instruction; (2) no `[ Style: editor, rewrite ]` prefill was provided, so Xialong defaulted to prose mode. The refine path now builds context from scratch — `buildStoryEnginePrefix` (pure story context, no generation directives) followed by `buildRefineTail` (divider, rewrite instructions, target snapshot, history) — mirroring the structure of the working brainstorm chat path. Xialong mode additionally receives a `[ Style: editor, rewrite, instructed ]` assistant prefill to anchor the generation voice.
