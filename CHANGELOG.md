...
- **The Engine watches what you write, and acts on it.** Switched on, it wakes a few seconds after each generation, reads whatever prose has appeared since it last looked, and asks a small model one question: what in the recorded World has your story now made wrong, left hanging, or settled? Then it does something about the answer — **rewriting the lorebook entry** of a character or place the prose has changed, **opening a Thread** for a commitment the prose has raised, **switching off** one the prose has settled, and **condensing** an entry that has sprawled. It is **off by default** — switch it on at **Setup → Engine → On**, where you also set **Delay (seconds)**, how long after a generation it looks (8), **Minimum new paragraphs**, how much new prose is worth a pass (1), the **Thread limit** (8), and **Condense entries over (paragraphs)** (5). **Every one of them belongs to the story you set it in**, not to Story Engine: turning the Engine on for one story does not turn it on for the next. A pass costs one small generation to read the situation, plus at most one entry rewrite, out of the same output budget your own writing draws on — it re-reads what is left of that budget before every single action, so a pass that cannot afford a rewrite defers it to the next one rather than taking it out of your next generation — and a deferred rewrite is still made against the prose that called for it, never against whatever scene it happens to land in a pass or two later. Switching off a resolved Thread is free and never waits. **The Engine never asks you to wait for it**: if the budget cannot cover a pass, or you are already generating something yourself, it steps aside silently and tries again on the next wakeup rather than queueing up and turning the status widget into a Continue button for work you did not ask for. Writing by hand wakes nothing — there is no generation to wake it — so the HUD's ⚡ is how you run a pass over prose you typed yourself.
- **It edits your lorebook, and this release has no restore button.** The first time the Engine touches any entry it keeps a copy of that entry exactly as you had it — text, keys, always-on flag and all — and never overwrites that copy afterwards, however many times it rewrites the entry later. **Nothing reads those copies back yet.** They are being kept so that a later release can offer them to you; until then your recourse for an edit you did not want is the story editor's own undo, which now moves the World with it. Two things bound what the Engine can reach: it only ever **rewrites** entries it manages, so an entry no Story Engine entity or Thread is bound to is your document and its text is never touched — and it never deletes anything in your lorebook. There is one place it reaches past that, and it is a switch rather than a rewrite: an entry you file into **SE: Threads** yourself, with no Thread behind it, is indistinguishable from one the Engine left behind, so undo or redo will switch it off. Keep such an entry in a category of your own, or switch it back on. The strongest thing it does to an entry it no longer wants firing is switch it off, which is one click to put back.
- **Condensing is the one thing the Engine does that can lose information.** Every other action adds something, opens something or switches something off; a condense rewrites a long entry shorter, and a detail dropped on the way out is a detail nothing downstream will notice is missing. The prompt is written to keep facts and spend only words, and three checks stand behind it: a result that is not actually shorter than the entry is thrown away, one that comes back under a third of the entry's length is thrown away as a summary rather than a compaction, and one that ran out of room mid-rewrite is thrown away rather than trimmed. A rejected condense leaves the entry exactly as it stood, and the Engine will not offer that entry again until it has grown by another paragraph. What triggers it is length alone, and you own the number: **Setup → Engine → Condense entries over (paragraphs)**, default 5, range 2–10. Raise it if you would rather the Engine left your entries alone.
- **A one-line Engine HUD you cannot lose.** A second panel, **Engine HUD**, shows a single fixed line meant to be read as a shape rather than as words: an icon for what the Engine is doing (switched off, watching, reading, working, waiting on output budget, or stuck), then three counts — how many paragraphs it has not read yet, how many Threads are still **open** (marking one satisfied brings the number down; the tooltip also says how many the story is carrying in total, which is what the limit counts), and how many lorebook entries it has rewritten (recounted from the branch itself whenever you undo or redo, and starting from zero again after a reload until you do) — then four bars of remaining output budget, and a **⚡** that runs a pass on the spot. **Every slot is an icon carrying a tooltip that says what its number counts**, so the line stays something you take in at a glance and the words are one hover away. The ⚡ is what you need when you are typing by hand, since nothing else wakes the Engine. It does nothing while the Engine is off, and pressing it during a pass is ignored rather than starting a second one. The panel can be minimised but never closed, which is the point: the Engine can never be quietly running behind a surface you have shut.
- **The Engine's account of itself is behind Story Engine Debug, and that switch now does only that.** The list each pass produces goes to the script log when **Story Engine Debug** is on in NovelAI's script settings; with it off the Engine says nothing and the HUD is the whole surface, which is what makes the HUD worth watching. The same setting used to also print a `NAISTORE` line for every internal action Story Engine took — many per keystroke — which buried the Engine's handful of lines in exactly the log you would have turned it on to read. That firehose has moved to its own setting, **Log every store action**, off by default and only worth touching if you are debugging Story Engine's state.
- **Threads have a scale and a lifecycle.** A Thread is the story's unfinished business — a promise made, a letter unopened, an argument left hanging — and until now every Thread was the same size and lasted forever. Each one now carries a **horizon** saying how much of the story it spans, set from three buttons in the Thread's edit pane: **Point** for a detail the next few paragraphs should pick up, **Plot** for a subplot that can survive a scene about something else, and **Arc** for the story's spine. A Thread also has a **status** — open, or satisfied once the story has settled it — shown on every row in the World list as a hollow circle or a check, so you can read the column without opening anything. Satisfied Threads stay exactly where you left them rather than being filed away, because reopening one means finding it first. You still set both by hand, and the Engine now sets them too; the one thing it never does is rescale a Thread, so every Thread it opens is a **Plot** and moving one to Arc or Point is yours.
- **A Thread now has a lorebook entry that stays quiet while you are still writing about it.** A Thread's job was always to keep its people within the model's reach, and it did that by being permanently switched on — spending context whether or not the story needed reminding. A Thread the Engine opens now gets an entry of its own, filed under **SE: Threads**, holding a one- or two-sentence note about what is still outstanding. The entry is not keyed on words at all — a condition alone decides when it speaks. As long as your prose keeps naming the Thread's cast or its subject, the note costs you nothing. Once they have gone missing for a while — a couple of paragraphs at Point, about a scene at Plot, about a chapter at Arc — it starts arriving. A freshly opened Thread also gets one quiet window before it may speak at all, so its first verdict is about prose written after the commitment existed rather than before it. **Every named Thread gets one**, not just the ones the Engine opens — a Thread from the Forge gets its entry as it is created, and one you add yourself gets it the moment you give it a title and Save. An untitled Thread gets nothing until then, so abandoning the "+ New Thread" pane leaves no stray entry behind. (The lorebook _sync_ toggle on a Thread card is a separate thing and is still greyed out.)
- **Threads now retire themselves, including the ones the story walked away from.** When your prose settles a commitment, the Engine marks that Thread satisfied and switches its entry off — the reminder simply stops, rather than the model being told a plot is over. A Thread nothing has mentioned for a long time ages out the same way: ten of its own forgetting windows (roughly 25 paragraphs at Point, 100 at Plot, 300 at Arc) with no sign of it in anything the Engine reads. Any pass that does find it in your prose pushes that clock back to the present, so a Thread you are circling never ages. A Thread you made by hand starts with no clock at all — the Engine has no idea when you last touched it — and gets one only when your prose first mentions it, which is a moment it can honestly age from. **A Thread that ages out is marked _abandoned_, not satisfied** — a third reading, with its own glyph in the World list, because a check against a commitment you never resolved would be the Engine telling you something untrue about your own story. Abandoned behaves exactly as satisfied does everywhere else: the entry goes quiet, the Thread frees its slot, and reopening it is one press. Marking a Thread satisfied by hand switches its entry off, and reopening one switches it back on, the same way the Engine's own retirement does. **Deleting a Thread switches its entry off too** — nothing in Story Engine names that entry any more, so nothing else would ever quieten it. The entry stays in your lorebook, one switch away from coming back.
- **A Thread limit, so unfinished business cannot pile up unchecked.** **Setup → Engine → Thread limit** caps how many Threads a story carries at once, default **8**. The World's add-Thread button now carries the count — `3/8` — so you can see where you stand without opening Setup, and **at the limit it will not create**: it dims, and its tooltip says to satisfy and delete a Thread or raise the limit. Nothing you wrote is spent on a button press. A Thread arriving from the Forge or from the Engine at the limit is a different case and does displace, because that create came with a decision attached: the weakest Thread goes — a satisfied one first, then the shortest horizon, then the oldest — and never the one just made. Lowering the limit deletes nothing by itself; it takes effect on the next Thread that is added. The displaced Thread's title and notes leave the World, and that is not undoable from inside Story Engine — but **its lorebook entry is switched off as it goes**, so a Thread nothing records any more cannot go on quietly reminding your model about it. The entry itself stays in your lorebook — its note, its keys and its condition untouched — one switch away from coming back.

### Changed

- **Story Engine's sidebar is now three tabs: Setup, Engine, and Chat.** Setup comes first and is where a story begins — it holds the narrative Foundation (Shape, Intent, Story Contract, ATTG, Style and Intensity) together with the **opening scene** control and the **Import Wizard**, both of which used to sit in the header. The header keeps the generation status widget and the S.E.G.A. progress line, and is quieter for it. The Engine tab keeps the World, Threads and the Forge. Opening a story with nothing written yet lands you on Setup; opening one already in progress lands you on Engine.
- **The Setup tab leads you through building a story rather than showing everything at once.** Import Wizard at the top for bringing existing material in, then Intensity, then a prompt to talk the story through, then the Foundation, then the opening scene, and the Engine's own settings last. The Foundation now lives in a collapsible section that starts closed and opens itself once a brainstorm has something in it — and stays however you leave it after that. The opening scene is a full card that appears once the Foundation has content and the story is still blank.
- **"Talk it through" opens a fresh brainstorm, and appears when you need it.** It sits directly under Intensity while there is nothing to work from, and steps aside as soon as there is — either a Foundation field filled in, or a brainstorm you have actually talked in. Starting a conversation about the story no longer means a trip to the Chat tab and a New chat button. Until you have picked an Intensity it waits, pointing back up at the picker — the brainstorm writes in the register you set, so choosing one first is worth the extra click.
- **Undo and redo now move the World with the story.** What Story Engine derives from your prose — the World and the story fields — is stored against the point in history it belongs to. Undo past the scene where a character was created and that character goes with it; redo and it comes back. Explore two continuations from the same point and each keeps its own World. Two things are deliberately exempt, because they belong to the story rather than to a moment in it: your brainstorms, so undoing a paragraph never hides the conversation that led to it, and the Foundation — Shape, Intent, Contract, ATTG and Style describe the whole story, and ATTG and Style feed Memory and Author's Note, which undo does not move either. Undo and redo now also reach into the lorebook for one thing the Engine owns: **a Thread's entry is switched on when the branch you land on holds that Thread open, and off when it does not.** Undo past the pass that retired a Thread and its reminder comes back; land on a branch where the Engine never opened it, or where the Thread limit displaced it, and the entry goes quiet instead of injecting a commitment that branch never made. Entry _text_ the Engine rewrote is not put back — Story Engine records enough to tell you whether an entry still says what it wrote, and deliberately not enough to overwrite it from — so an entry you have edited yourself since is never touched.
- **Keys and summaries are now generated by the model that is good at them.** Story Engine uses two models: one tuned for prose, one that follows instructions more reliably. Activation keys and the short internal summaries behind each entity and Thread are extraction work — nobody reads them as writing — so they now always go to the instruction-following model, even with Xialong Mode on. In practice that means fewer key lists that arrive with commentary attached. Everything you actually read is unchanged and still written by the prose model: the opening scene, lorebook entry text, chat, the Forge, and the Foundation fields.
...
- **Story Engine's saved state does not carry over from 0.14.x.** The storage layout changed so that undo can move the World, and Story Engine is still alpha, so there is no migration — on the first open after upgrading, the World, the Foundation and your saved brainstorms are all empty — **including every Thread you have created**, which this release also rebuilt to carry a horizon and a status (they move to new storage locations, not to a converted version of the old one). Your story text, lorebook, Memory and Author's Note are untouched; what resets is Story Engine's own record of which entries it manages, and the Import Wizard picks that back up.
...
- **Generated and refined text no longer stops mid-sentence.** A generation that ran out of room was saved exactly where the cap fell — a lorebook entry breaking off mid-word, a refined field committing the fragment it had reached. Both now continue automatically whenever the model was cut off rather than finished, carrying on until the text reaches a natural end or a generous ceiling; an entry that fits in one pass still costs a single generation. Refining a lorebook entry also gets a budget sized to the entries it rewrites (previously it was given less than half the room the entry was written with, so a full rewrite could never fit), and no longer treats a closing bracket as the end of the entry — so prose containing a bracketed aside survives intact. Stopping S.E.G.A. reaches an entry that is mid-continuation, rather than letting it run on.
- **The Forge no longer litters your lorebook with duplicate categories.** Committing a forge session casts every draft at once, and each one created its own `SE: Characters` (or `SE: Locations`, …) instead of sharing one — so a single commit could leave six identically-named categories behind, with the entities scattered across them. Casting is now a true find-or-create: drafts landing in the same category at the same moment share the one category. Story Engine also tidies up on load, folding any duplicate `SE:` categories an earlier build left behind into a single category and moving their entries across. Your own categories are never merged, even if two share a name.
- **Generation can be un-stalled again.** NovelAI's script harness pauses token replenishment when it can't see recent user activity, and clicks inside a script's own panel didn't count — so the output budget drained and never refilled, and the Continue button was decorative: a paused generation stayed paused. The harness now registers interaction inside the panel, so simply working in Story Engine keeps the budget topping up, and one press on the header's status widget resumes a stalled generation.
...
- **The Forge button can no longer open two sessions at once.** A single press could start two forges generating side by side, burning double the budget and leaving a stray session behind. A new forge is not visible to the rest of the app until its briefing has finished building, so a second press in that gap saw no session open and started another — a window far longer than a stray tap. Forge now refuses to open a second session while one is still starting, and the button ignores a repeat press within the same tap.
- **An unsent chat message is no longer lost.** Switching to the Story Engine tab tore down the chat panel and took the composer's contents with it, so a long setup message written but not yet sent was simply gone on the way back. Unsent text is now kept per chat and saved with the story, so it survives tab switches, session switches, and reloads alike — and each session keeps its own, so switching chats no longer shows you the previous one's half-written message. Sending or clearing drops the draft straight away.
- **One tap on Sum writes one summary.** A single press of **Sum** could open two summary sessions over the same brainstorm, each generating its own summary against the same output budget — the chat header's controls were the last panel with no protection against a mobile tap arriving twice. Every control up there now ignores a repeat inside the same tap (Sum, Co/Crit, the forge phase pills, New chat, Sessions, and Back), and the same goes for the Sessions list, where a doubled tap could open two brainstorms or — because rows are patched in place as the list changes — delete a session other than the one aimed at. Summarizing is also guarded underneath the buttons: while a summary of the same source is still being written, asking again brings you back to it instead of starting a second one. Summarizing again once it has finished still writes a fresh summary, as before.
- **The Story Contract generates a contract instead of more story.** Asking for a Story Contract returned a passage of prose — the story carrying on from wherever it had reached — rather than the Required / Prohibited / Emphasis lines. The request was assembled entirely from background context with no actual question at the end of it, so the model had nothing to answer and continued the story text sitting in that context instead. It is now asked outright and starts its reply from the `REQUIRED:` label, so it can only answer in the contract's three-line form.
- **The Import wizard imports the Story Contract.** Importing an existing story filled in ATTG, Style, Shape and Intent but silently skipped the Contract, leaving it the one Foundation field you had to go back and fill in by hand. **Import All** now writes it too, and there is a **Contract** button in the Story row beside Shape and Intent. It runs after those two, so it is drafted from the Shape and Intent the same import just produced.
- **Saving an entity can no longer leave a duplicate lorebook entry behind.** Saving a new entity for the first time is what creates its lorebook entry, and a doubled tap on **Save** — or on the Content ⚡ when it opens a refine chat — could start that twice, so the second entry took the entity and the first was stranded in your lorebook as an untracked copy. Both paths now refuse to run a second time while the first is still going, so one press writes one entry however long the save takes.
- **The regenerate ⚡ on an entity card no longer generates everything twice.** The bolt dims while it works, but on a doubled tap the second click landed before it dimmed, and the check it makes against your lorebook takes long enough that both presses saw the same missing summary, content, and keys — queueing all three twice, at double the budget, each pair racing to write the same entry. Repeat presses are ignored, and any piece already queued or in progress (including one S.E.G.A. is working on) is now skipped rather than started again.
- **Adding an entity or a thread no longer adds two.** A single tap on **+** in the World header could be delivered as two clicks, creating a pair of blank entries and opening the editor on the second — so the first was left behind as a stray `(unnamed)` entity or empty thread you had to hunt down and delete. Both buttons now ignore a repeat click inside the same tap.
- **Importing from an existing story no longer imports it twice.** A single tap on **Import All** could be delivered as two clicks, so the wizard ran its whole batch again — Shape and Intent were each generated twice, side by side, against the same output budget. Every control in the Import wizard now ignores a repeat click inside the same tap: Import All, Refresh and Back, the individual Memory → ATTG and A/N → Style imports, the Shape and Intent buttons, and each lorebook row's Bind and category picker, where a doubled tap could adopt one entry twice and leave two copies of the same character in your World. Both hazards are guarded underneath the buttons as well — a lorebook entry can only ever belong to one Story Engine entity, and a Foundation field refuses a second generation while one for that field is already queued or running, so nothing can queue the same work twice however it was asked for.
- **A single tap can no longer delete two chat messages, or retry the wrong one.** Retry and Delete on a chat bubble could each fire twice from one tap. Because the message list is patched positionally, the second click landed on whichever message had just shifted into that slot — so deleting one message could take its neighbour with it, and Retry could regenerate against the wrong turn while pruning the reply it had itself just queued. The forge and refine **Commit** / **Discard** bars had the same shape: they resolve the current session at click time, so a repeat could act on a different chat once the first had closed.
- **Switches and expanders respond to touch.** On phones and tablets, anything holding an on/off state was hard to operate: one tap could deliver two clicks, applying the new state and immediately undoing it. The ATTG and Author's Note sync switches looked stuck (and wrote to Memory / Author's Note twice), a collapsed **Context** block in chat refused to open, and the World section, expand/collapse-all, thread collapse, thread membership, and an entity's Always On all behaved the same way. These now ignore a repeat click arriving inside the same tap — as do Foundation's ⚡ and the S.E.G.A. start/stop, where a doubled tap could queue two generations or start and instantly stop a run.
...
- **Brainstorm Cowriter is a warmer collaborator at low intensity.** At Cozy and Grounded the co-writer now reacts to your idea and builds on it — offering suggestions as "what if…" rather than barking "Give her…/Don't give her…". Higher registers (Gritty → Nightmare) stay direct and unsoftened, and the Critic is unchanged. Cozy was also tightened toward pure warmth (no more "optional push-pull" between characters).
- **Slimmer prompts: brainstorm and field generation no longer carry worldbuilding boilerplate.** Every Story Engine generation used to be prefixed with the lorebook/entity-generation system prompt ("create potential for conflict," container rules, cross-referencing) regardless of task. That bundle now lives only where it's used — the lorebook Archivist — so brainstorming, foundation fields, and the opening scene run on their own focused instructions. This removes a conflict bias and a stray `[SYSTEMS]`-style tag leak from brainstorm, and trims tokens from every call.
