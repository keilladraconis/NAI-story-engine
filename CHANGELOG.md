...
- **The Engine watches what you write and records what needs attention — and does nothing else yet.** Switched on, it wakes a few seconds after each generation, reads whatever prose has appeared since it last looked, and asks a small model one question: what in the recorded World has your story now made wrong, left hanging, or settled? The answer is a short list — revise this character, open this unresolved commitment, retire this thread — and that is where it stops. **Nothing is written, created, or retired.** Your lorebook, your entities and your Threads are untouched; this release ships the watching half deliberately, so you can read what the Engine would do for a few sessions before it is allowed to do any of it. It is **off by default** — switch it on at **Setup → Engine → On**, where you can also set **Delay (seconds)**, how long after a generation it looks (8), and **Minimum new paragraphs**, how much new prose is worth a pass (1). **All three belong to the story you set them in**, not to Story Engine: turning the Engine on for one story does not turn it on for the next, and each story keeps its own delay and threshold. A pass costs one small generation out of the same output budget as everything else, and skips itself when there is nothing new to read. Writing by hand wakes nothing — there is no generation to wake it — so the HUD's ⚡ is how you run a pass over prose you typed yourself.
- **A one-line Engine HUD you cannot lose.** A second panel, **Engine HUD**, shows a single fixed line meant to be read as a shape rather than as words: an icon for what the Engine is doing (switched off, watching, reading, working, waiting on output budget, or stuck), then three counts — how many paragraphs it has not read yet, how many Threads are still **open** (marking one satisfied brings the number down; the tooltip also says how many the story is carrying in total, which is what the limit counts), and how many entities it has revised on this branch (always zero while it only watches) — then four bars of remaining output budget, and a **⚡** that runs a pass on the spot. **Every slot is an icon carrying a tooltip that says what its number counts**, so the line stays something you take in at a glance and the words are one hover away. The ⚡ is what you need when you are typing by hand, since nothing else wakes the Engine. It does nothing while the Engine is off, and pressing it during a pass is ignored rather than starting a second one. The panel can be minimised but never closed, which is the point: the Engine can never be quietly running behind a surface you have shut.
...
- **Undo and redo now move the World with the story.** What Story Engine derives from your prose — the World and the story fields — is stored against the point in history it belongs to. Undo past the scene where a character was created and that character goes with it; redo and it comes back. Explore two continuations from the same point and each keeps its own World. Two things are deliberately exempt, because they belong to the story rather than to a moment in it: your brainstorms, so undoing a paragraph never hides the conversation that led to it, and the Foundation — Shape, Intent, Contract, ATTG and Style describe the whole story, and ATTG and Style feed Memory and Author's Note, which undo does not move either.
- **Threads are built to speak up when you forget them — but nothing has switched that on yet.** A Thread's job was always to keep its cast within the model's reach, and it did that by being permanently switched on, which spends context whether or not the story needs reminding. The replacement is a Thread that stays quiet while its people and its subject are still turning up in your prose, and speaks only once they have gone missing from it — for a couple of paragraphs at Point, about a scene at Plot, about a chapter at Arc. **This release builds and tests that trigger and connects it to nothing.** Threads still have no lorebook entry of their own, the sync control on a Thread card is still greyed out, and your lorebook behaves exactly as it did in 0.14.x. What ships is the machinery, so that the release which turns it on is a small change rather than a leap.
