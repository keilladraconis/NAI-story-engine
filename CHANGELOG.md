...
- **The chat input adapts to the active chat type.** A forge session shows the placeholder "Discuss, instruct, or send empty to Forge Ahead…", the ⚡ Forge Ahead / Send button above, and no Clear button; brainstorm/summary keep the generic input. Driven by `inputPlaceholder` / `sendLabel` / `showClearButton` fields on `ChatTypeSpec`, with the forge send label resolved live from input emptiness.
