...
- **A one-line Engine HUD you cannot lose.** A second panel, **Engine HUD**, shows a single fixed line meant to be read as a shape rather than as words: an icon for what the Engine is doing (switched off, watching, reading, working, waiting on output budget, or stuck), then three counts — how many paragraphs it has not read yet, how many Threads the story is carrying, and how many entities it has revised on this branch (always zero while it only watches) — then four bars of remaining output budget, and a **⚡** that runs a pass on the spot. **Every slot is an icon carrying a tooltip that says what its number counts**, so the line stays something you take in at a glance and the words are one hover away. The ⚡ is what you need when you are typing by hand, since nothing else wakes the Engine. It does nothing while the Engine is off, and pressing it during a pass is ignored rather than starting a second one. The panel can be minimised but never closed, which is the point: the Engine can never be quietly running behind a surface you have shut.
