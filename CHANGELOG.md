...
- **It edits your lorebook, and it keeps no copy of what it replaced.** When the Engine rewrites an entry, the wording you had is gone: Story Engine stores nothing you could put back, and undo will not restore it either. **If the exact text of an entry matters to you, export your lorebook before you switch the Engine on.** What bounds the damage is how little it reaches for. It only ever **rewrites** entries it manages, so an entry that no Story Engine entity or Thread is bound to is your document and its text is never touched. It never deletes anything in your lorebook, and never touches an entry's category. The strongest thing it does to an entry it no longer wants firing is switch it off, which is one click to put back.
- **Story Engine's records move forward only.** Undo changes your prose; it does not change what Story Engine has written down. A character, a Thread or a field created during a scene you have since backed out of is still in the World when you get back there, and **deleting it is the only thing that takes it out**. The Engine's own work moves forward the same way: an entry it rewrote stays rewritten, a Thread it opened stays open, and a Thread's entry it switched off stays off. That is the trade for a World that is a notebook rather than a mirror of the page — two continuations from the same point share one set of notes, so exploring a branch where a character dies leaves the World saying she is dead until you say otherwise.
