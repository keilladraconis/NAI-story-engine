...
- **A Thread limit, so unfinished business cannot pile up unchecked.** **Setup → Engine → Thread limit** caps how many Threads a story carries, default **8**. The World's add-Thread button carries the count — `3/8` — and **at the limit it will not create**: it dims, and its tooltip says to satisfy and delete a Thread or raise the limit. A Thread arriving from the Forge or the Engine does displace, because that create came with a decision attached: the weakest goes — a satisfied one first, then the shortest horizon, then the oldest — and never the one just made. Lowering the limit deletes nothing; it applies to the next Thread added. A displaced Thread's title and notes leave the World, but **its entry is switched off as it goes**, and the entry itself stays in your lorebook untouched.

### Changed

- **Story Engine's sidebar is now three tabs: Setup, Engine, and Chat.** Setup is where a story begins — the narrative Foundation (Shape, Intent, Story Contract, ATTG, Style and Intensity) with the **opening scene** control and the **Import Wizard**. The header keeps the generation status widget and the S.E.G.A. progress line. The Engine tab holds the World, Threads and the Forge. Opening a story with nothing written lands you on Setup; opening one in progress lands you on Engine.
