...
- **The forge chat is now agentic.** Typing in the input *discusses* with the forge — it answers conversationally and only emits actions for what you explicitly ask (e.g. "add a dockmaster named Halloran"). Running the semi-automatic expansion pass moved to a **Forge Ahead** button in the header; an empty send no longer forges. A **🧹 Scrub** header indicator appears when discards have queued a reference-cleanup and runs it on demand without a full pass. Emitted actions show as their (canonical) command lines in the assistant bubble for now, alongside the inline draft cards — they'll become compact chips in a later pass. The send button is now "Send".
