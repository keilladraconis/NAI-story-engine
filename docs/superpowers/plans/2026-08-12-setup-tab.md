...
`header-model.ts` is **not** touched in this task — it keeps its own copy of `selectBootstrapPending` and `BOOTSTRAP_TYPES` until Task 5, so this task is a pure addition that cannot break the running UI.

**The duplication is deliberate and time-boxed, not an oversight.** Deleting the header's copy here would break `Header.tsx`, which still imports it, and rewiring that import to `panels/setup/` would create a header→panel dependency that Task 5 immediately deletes again. Task 5 Step 5.3 removes the header's copy, leaving one definition. A reviewer seeing duplicated logic in this task's diff should read it as scheduled, not as a defect.
