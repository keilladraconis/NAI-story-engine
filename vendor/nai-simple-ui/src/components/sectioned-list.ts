...
import {
  type SuiSectionedListStateTheme,
  type SuiSectionedListTheme,
} from "./theme/sectioned-list.ts";
 ...
 */
export class SuiSectionedList extends SuiComponent<
  SuiSectionedListTheme,
  Record<string, unknown>,
  SuiSectionedListOptions,
  UIPartColumn
> {
  ...
  override filter(query: string): SuiFilterResult {
    ...
    updates.push({
      id: this.id,
      style: anyVisible ? full : { ...full, display: "none" },
    });
  ...
  async compose(): Promise<UIPartColumn> {
    ...
    const sections: AnySuiComponent[] = this.options.sections.map(
      (section, i) => {
        ...
        if (section.label !== undefined) {
          ...
          shellChildren.push(
            new SuiText({
              id: headerId,
              theme: {
                default: { self: { ...t.header, text: section.label } },
              },
            }),
          );
        }
